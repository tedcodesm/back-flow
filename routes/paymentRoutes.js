  ...
  try {
    ...
    if (order) {
      if (order.status === "completed") {
        return res
          .status(400)
          .json({ message: "You already paid for this property" });
      }
        ...
        try {
          ...
          if (session.status === "open") {
            return res.json({
              ...
              message: "Continuing previous payment session",
            });
          }
          ...
          console.log("Old session invalid:", err.message);
        }
      }
...
// Success handler – called from frontend after redirect
router.post("/success", protect, async (req, res) => {
  try {
    const { session_id } = req.body; // or req.query if GET – but POST is safer
    ...
    // You can store more: last4, receipt_url, etc.
    // order.receiptUrl = session.payment_intent?.receipt_url;  (if you retrieve PaymentIntent)
    ...
    res.status(200).json({
      success: true,
      message: "Payment successful, order completed, property reserved.",
      orderId: order._id,
    });
  } catch (error) {
